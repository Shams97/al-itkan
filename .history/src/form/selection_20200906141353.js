...
export default function Radio ({name, ops, classes, values, label}) {
  ...
  return (
    ...
    className={"border-b-2 border-black-400 focus:border-blue-500 outline-none font-serif bg-none " + (!!classes ? classes: "")}>
      <option  value="" disabled selected> anything </option>
      {ops.map((op, index) => {
