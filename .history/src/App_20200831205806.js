...
export const FormInput = ({placeholder, name, all_value, type, textarea})=>{
  ...
  const handleChange = (e) => {
    ...
    if (type === 'file') {
      ...
      all_value = e.target.value
      state['data'][name] = all_value
    }
    console.log(state)
    setState(state)
  }
  ...
  if (!!textarea){
    return (
      <textarea type={type} placeholder={placeholder} name={name} onChange={handleChange} value={all_value}
      className="pb-4 border-b-2 border-black-400 focus:border-blue-500  outline-none p-2" />
    )
  } else {
    return (
      <input type={type} placeholder={placeholder} name={name} onChange={handleChange}  value={all_value}
      className="border-b-2 border-black-400 focus:border-blue-500 outline-none p-2"  />
    )
  }
}
