...
export default function Jobs(vacancy) {
  ...
  return (
    (vacancy = "anything"),
    (
      <div className="max-w-full flex flex-col flex-wrap  justify-center">
        {jobState.map((item, index) => {
          return (
            <div
              className="rounded overflow-hidden bg-white shadow-lg  mx-auto mt-20 w-1/4 md:w-1/4 "
              key={index}
            >
              <img src={src[index]} className="object-cover h-50 w-1/4"/>
              <div className="wave"/>
              ...
              <div className="py-10 px-4 ">
                ...
                <div className="flex flex-row justify-between">
                  <Link to={{ pathname: `/personal/job=${item.job_name}` }}>
                    <button
                      className="px-4 py-1 bg-green-600 rounded text-white mb-2 "
                      name={item.job_name}
                      onClick={handleClick}
                    >
                      Apply now
                  ...
                  <p className="text-red-600 text-sm font-sans"> <span className="font-sans text-black">Ends in</span> {item.dead_line}</p>
                </div>
              </div>
            </div>
          );
