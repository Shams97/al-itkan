  ...
  return (
    <>
      ...
      <div className="max-w-full flex flex-row flex-wrap  justify-center ">
          ...
          loading={loading}
        />
                   <p className="flex justify-center  mx-auto">Jobs still loading ... </p>
          </div>
           :jobState.length===0 ? 
           (
        ...
        jobState.length === 0 ? (
          ...
          jobState.map((item, index) => {
            return loading ? <div className="opacity-25 bg-gray-500 "><p className="flex justify-center mt-32 mx-auto">This page is still loading </p></div>
              :  item.state != "open"  ? (
              <div
                className="rounded overflow-hidden bg-white shadow-lg  mx-4 mt-20 md:w-1/4 lg:w-1/4 sm:1/2 "
                key={index}
